...
from models import FullyConvNet, FullyConvResNet, UNet, UNetBidirection, UNetBidirection2, UNetBidirection3, DilateResNetBidirection, UNetBidirection4
...
def train_unsupervised(args, model, m_kernel, reverse_m_dict):
    ...
    for epoch in range(args.train_epoch):
        ...
        if (epoch+1) % args.test_interval == 0:
            ...
            save_model(args, model, m_kernel, reverse_m_dict, best_improve_percent)
    return model


def save_model(args, model, m_kernel, reverse_m_dict, best_improve_percent):
...
def test_unsupervised(args, model, m_kernel, reverse_m_dict):
    ...
    for epoch in range(args.test_epoch):
        ...
        if args.display:
            ...
            visualize(im_input_f, im_input_b, im_output, im_pred, flow_f, disappear_f, attn_f, flow_b, disappear_b, attn_b)
...
def test_one_video(args, model, sub_dir, sub_sub_dir, m_kernel, reverse_m_dict):
    ...
    for i in range(len(files)):
        ...
        if i < args.num_inputs:
            ...
            flow = numpy.zeros((im_height, im_width, 2))
        else:
...
def main():
    ...
    # model = DilateResNetBidirection(height, width, channel, num_inputs, len(m_dict), m_range, m_kernel)
