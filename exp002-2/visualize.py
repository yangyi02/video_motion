...
def visualize(im_input, im_output, pred, pred_motion, disappear, m_range, reverse_m_dict):
    img = visualize_image(im_input, im_output, pred, pred_motion, disappear, m_range, reverse_m_dict)
    plt.figure(1)
    plt.imshow(img)
    plt.axis('off')
    plt.show()
...
def visualize_image(im_input, im_output, pred, pred_motion, disappear, m_range, reverse_m_dict):
    im_height, im_width = im_output.size(2), im_output.size(3)
    width, height = get_img_size(2, 5, im_width, im_height)
    img = numpy.ones((height, width, 3))

    im1 = im_input[0, -6:-3, :, :].cpu().data.numpy().transpose(1, 2, 0)
    x1, y1, x2, y2 = get_img_coordinate(1, 1, im_width, im_height)
    img[y1:y2, x1:x2, :] = im1

    im2 = im_input[0, -3:, :, :].cpu().data.numpy().transpose(1, 2, 0)
