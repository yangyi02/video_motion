...
def generate_batch_old(args, meta):
...
def generate_batch(args, meta):
    ...
    while i < batch_size:
        ...
        for j in range(n_inputs - 1):
            diff = im_input[i, j*im_channel:(j+1)*im_channel, :, :] - im_input[i, (j+1)*im_channel:(j+2)*im_channel, :, :]
            diff = numpy.abs(diff)
            diff = numpy.sum(diff) / args.num_channel / im_height / im_width
            im_diff.append(diff)
        diff = numpy.abs(im_input[i, -im_channel:, :, :] - im_output[i, :, :, :])
        diff = numpy.sum(diff) / args.num_channel / im_height / im_width
        im_diff.append(diff)
        cnt = cnt + 1
        if im_diff[-1] < 0.01:
        ...
        if any(im_diff_div > 2) or any(im_diff_div < 0.5):
            continue
        i = i + 1
    return im_input, im_output
