...
from models import FullyConvNet, FullyConvResNet, UNet, DilateResNet
from visualize import visualize, get_img_size, get_img_coordinate
...
def validate(args, model, m_kernel, reverse_m_dict, best_improve_percent):
...
def test_unsupervised(args, model, m_kernel, reverse_m_dict):
    ...
    for epoch in range(args.test_epoch):
        ...
        if args.display:
            ...
            visualize(im_input, im_output, im_pred, flow, m_range, reverse_m_dict)
...
def test_video(args, model, m_kernel, reverse_m_dict):
    ...
    for sub_dir in os.listdir(test_dir):
        ...
        for sub_sub_dir in os.listdir(os.path.join(test_dir, sub_dir)):
            if not os.path.exists(os.path.join(flow_dir, sub_dir, sub_sub_dir)):
                os.mkdir(os.path.join(flow_dir, sub_dir, sub_sub_dir))
            test_one_video(args, model, sub_dir, sub_sub_dir, m_kernel, reverse_m_dict)
...
def test_one_video(args, model, sub_dir, sub_sub_dir, m_kernel, reverse_m_dict):
    ...
    for i in range(len(files)):
        image_file = os.path.join(args.test_dir, sub_dir, sub_sub_dir, files[i])
        ...
        im_input[0, :-im_channel, :, :] = im_input[0, im_channel:, :, :]
        im_input[0, -im_channel:, :, :] = im[:, idx_h:idx_h+height, idx_w:idx_w+width]
        if i < args.num_inputs:
            ...
            motion = model(im_input_torch)
            m_mask = F.softmax(motion)
            flow = motion2flow(m_mask, reverse_m_dict)
            flow = flow[0].cpu().data.numpy().transpose(1, 2, 0)
        if args.display:
            im_width, im_height = get_img_size(1, 2, width, height)
            img = numpy.ones((im_height, im_width, 3))

            x1, y1, x2, y2 = get_img_coordinate(1, 1, width, height)
            img[y1:y2, x1:x2, :] = im_input[0, -im_channel:, :, :].transpose(1, 2, 0)

            optical_flow = flowlib.visualize_flow(flow)
            x1, y1, x2, y2 = get_img_coordinate(1, 2, width, height)
        ...
        flow_file = os.path.join(args.flow_dir, sub_dir, sub_sub_dir, file_name)
