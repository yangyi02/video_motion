...
def visualize(im_input, im_output, pred, pred_motion, m_range, reverse_m_dict):
    img = visualize_image(im_input, im_output, pred, pred_motion, m_range, reverse_m_dict)
    plt.figure(1)
    plt.imshow(img)
    plt.axis('off')
    plt.show()
...
def visualize_image(im_input, im_output, pred, pred_motion, m_range, reverse_m_dict):
    ...
    im2 = im2 * 255.0
    next_frame = cv2.cvtColor(im2.astype(numpy.uint8), cv2.COLOR_RGB2GRAY)
    im3 = im3 * 255.0
    prvs_frame = cv2.cvtColor(im3.astype(numpy.uint8), cv2.COLOR_RGB2GRAY)
