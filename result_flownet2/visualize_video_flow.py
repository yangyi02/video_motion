...
def create_video(video_list, flow_list, output_folder='flow_video'):
    if not os.path.exists(output_folder):
        os.mkdir(output_folder)
    for key in video_list.keys():
        video_files = video_list[key]
        flow_files = flow_list[key]
        create_one_video(video_files, flow_files, os.path.join(output_folder, key + '.avi'))


def create_one_video(video_files, flow_files, flow_video_file):
    vid = None
    for i in range(len(video_files)):
        if i == 0:
            im = numpy.array(Image.open(video_files[i]))
...
def main():
    ...
    create_video(video_list, flow_list)
